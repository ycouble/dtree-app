...
XMIND_FILE = "data/salmonelles.xmind"
...
class DTree:
    def __init__(self, filename=XMIND_FILE):
        ...
        self.root_node = DTree._populate_node(
            self.raw_json_data[0]["rootTopic"])
    ...
    def _populate_node(data):
        ...
        children = []
        if "children" in data:
            children = [DTree._populate_node(child)
                        for child in data["children"]["attached"]]
            if "summary" in data["children"]:
                valid_children = (
                    child for child in children if len(child.children) == 0)
                summaries = [DTree._populate_node(
                    summary) for summary in data["children"]["summary"]]
    ...
    def create_relationships(self, relationships):
        for relation in relationships:
            ...
            link = Link(relation['id'], relation.get(
                'title', ""), start_node, end_node)
            if not start_node or not end_node:
                import pdb
                pdb.set_trace()  # TODO error management
                print(relation['end1Id'], relation['end2Id'])
            start_node.links.append(link)
            links = (link for link in start_node.links if len(
                start_node.links) > 1)
    ...
    def print_raw_json(self):
        print(json.dumps(self.raw_json_data,
